...
const Login=()=>{
    ...
    return (
        <div className="relative">
           ...
           <form onSubmit={(e)=>e.preventDefault()} className="absolute top-0 left-0 right-0 mt-32 mx-auto px-16 pt-8 pb-20 bg-[#100f0f] opacity-85 text-white flex flex-col w-4/12 rounded-lg">
             <h2 
              className="m-3 text-3xl font-bold"
              >
              {isSignIn ? "Sign In" : "Sign Up"}
              </h2>
             {!isSignIn && <input ref={name} className="mt-4 p-4 rounded-lg bg-[#0f0e0e] border-white border-2" type="text" placeholder="Name"/>}
             <input 
              ref={email}
